...
export default function ProjectGallery({ studies }: { studies: CaseStudy[] }) {
  return (
    <ul className="grid gap-6 overflow-x-clip sm:grid-cols-2 lg:grid-cols-3">
      {studies.map((study, i) => (
        <GalleryCard key={study.slug} study={study} index={i} />
      ))}
    </ul>
  );
}

/* One card. Hovering anywhere on it, the screen, the copy or the button,
   runs the plate's tag reveal, so the card answers as one object. */
function GalleryCard({ study, index: i }: { study: CaseStudy; index: number }) {
  ...
  return (
    <li
      ...
      onFocus={(e) =>
        e.currentTarget.matches(':focus-within') && setActive(true)
      }
      onBlur={(e) =>
        !e.currentTarget.contains(e.relatedTarget as Node) && setActive(false)
      }
    >
      <Link
